...
public class NearestNeighbourSymbolFromImageRecognizerTest {
    ...
    public void classify() throws IOException, IM3Exception {
        ...
        TreeSet<RankingItem<SymbolImagePrototype>> rank = nearestNeighbourSymbolFromImageRecognizer.classify(symbolImagePrototype);

        // the exact match should be obtained
        assertEquals("Best match", "note.wholeBlack-S4", rank.first().getClassType().getPrototypeClass().getAgnosticString());
    }
}