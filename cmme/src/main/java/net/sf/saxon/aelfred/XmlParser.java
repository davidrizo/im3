...
/*
 * $Id: XmlParser.java,v 1.8 2001/06/06 17:57:44 dbrownell Exp $
...
final class XmlParser
{
     ...
     * @exception Exception Basically SAXException or IOException
     */
    // package private 
    void doParse (
	String		systemId,
...
loop:
	    while (atts.hasMoreElements ()) {
		    ...
		    if (defaultVal!=null) {
		        handler.attribute (aname, defaultVal, false);
	        }
	    }
	}
	...
	if (type == ATTRIBUTE_ENUMERATED || type == ATTRIBUTE_NOTATION) {
	    _enum = dataBufferToString ();
	}

	// Read the default value.
	requireWhitespace ();
	parseDefault (elementName, name, type, _enum);
    }
	...
	if (tryRead ('#')) {
	    ...
	    value = readLiteral (flags);
	setAttribute (elementName, name, type, _enum, value, valueType);
    }
...
loop:
	...
	if (c == '\n') {
		...
		if (peIsError && entityStack.size()==1)
		    // not an error if PE reference is in an external PE called from internal subset
	...
	if (!isAbsolute) {
        ...
        try {
    	    ...
    	    popInput();
            error("Invalid URL " + systemId + " (" + err.getMessage() + ")");
    	}
    }
	...
	if (reader == null && stream == null && systemId != null) {
	    ...
	    try {
	        ...
	        popInput();
	        error("Failure resolving entity " + systemId + " (" + err.getMessage() + ")");
	    }
	    if (input != null) {
	...
	if (stream != null) {	    
        ...
        try {
	        ...
	        error("Cannot read input file " + err.getMessage());
	    }
	}
	...
	if (encoding == null && externalEntity != null) {
		...
		if (temp < 0)
		    ...
		    encoding = encoding.substring (temp+1);   // +1 added by MHK 2 April 2001
	...
	*/
	...
	while (i < count) {
		...
		if ((b1 & 0xe0) == 0xc0) {
		    ...
		    encodingError (
			    "invalid UTF-8 byte (check the XML declaration)",
			    0xff & b1, i);
		    // NOTREACHED
		    c = 0;
		}
